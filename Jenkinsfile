        ...
        withCredentials([
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:openbox-focal
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:${META_TAG}
                  docker tag ${GITHUBIMAGE}:arm32v7-${META_TAG} ${GITHUBIMAGE}:arm32v7-openbox-focal
                  docker tag ${GITHUBIMAGE}:arm64v8-${META_TAG} ${GITHUBIMAGE}:arm64v8-openbox-focal
                  docker push ${GITHUBIMAGE}:amd64-${META_TAG}
                  docker push ${GITHUBIMAGE}:arm32v7-${META_TAG}
                  docker push ${GITHUBIMAGE}:arm64v8-${META_TAG}
                  docker push ${GITHUBIMAGE}:openbox-focal
                  docker push ${GITHUBIMAGE}:${META_TAG}
                  docker push ${GITHUBIMAGE}:arm32v7-openbox-focal
                  docker push ${GITHUBIMAGE}:arm64v8-openbox-focal
               '''
          }
          sh '''#! /bin/bash
                for DELETEIMAGE in "${GITHUBIMAGE}" "${GITLABIMAGE}" "${IMAGE}"; do
