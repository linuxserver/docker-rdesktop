        ...
        withCredentials([
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:openbox-bionic
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:${META_TAG}
                  docker tag ${GITHUBIMAGE}:arm32v7-${META_TAG} ${GITHUBIMAGE}:arm32v7-openbox-bionic
                  docker tag ${GITHUBIMAGE}:arm64v8-${META_TAG} ${GITHUBIMAGE}:arm64v8-openbox-bionic
                  docker push ${GITHUBIMAGE}:amd64-${META_TAG}
                  docker push ${GITHUBIMAGE}:arm32v7-${META_TAG}
                  docker push ${GITHUBIMAGE}:arm64v8-${META_TAG}
                  docker push ${GITHUBIMAGE}:openbox-bionic
                  docker push ${GITHUBIMAGE}:${META_TAG}
                  docker push ${GITHUBIMAGE}:arm32v7-openbox-bionic
                  docker push ${GITHUBIMAGE}:arm64v8-openbox-bionic
               '''
          }
          sh '''#! /bin/bash
                for DELETEIMAGE in "${GITHUBIMAGE}" "${GITLABIMAGE}" "${IMAGE}"; do
