        ...
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for PUSHIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  ...
                  docker buildx imagetools create -t ${PUSHIMAGE}:ubuntu-kde ghcr.io/linuxserver/jenkins-builder:empty || true
                done
              '''
        }
      }
      ...
      steps {
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  docker buildx imagetools create -t ${MANIFESTIMAGE}:ubuntu-kde -t ${MANIFESTIMAGE}:amd64-ubuntu-kde -t ${MANIFESTIMAGE}:arm64v8-ubuntu-kde ghcr.io/linuxserver/jenkins-builder:empty || true
