        ...
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for PUSHIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  ...
                  docker buildx imagetools create -t ${PUSHIMAGE}:ubuntu-openbox ghcr.io/linuxserver/jenkins-builder:empty || true
                done
              '''
        }
      }
      ...
      steps {
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  docker buildx imagetools create -t ${MANIFESTIMAGE}:ubuntu-openbox -t ${MANIFESTIMAGE}:amd64-ubuntu-openbox -t ${MANIFESTIMAGE}:arm64v8-ubuntu-openbox ghcr.io/linuxserver/jenkins-builder:empty || true
      ...
      steps {
        sh '''#! /bin/bash
              ...
              jq -n \
                ...
                --arg target_commitish "ubuntu-openbox" \
