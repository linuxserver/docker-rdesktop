        ...
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for PUSHIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  ...
                  docker buildx imagetools create -t ${PUSHIMAGE}:alpine-icewm ghcr.io/linuxserver/jenkins-builder:empty || true
                done
              '''
        }
      }
      ...
      steps {
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  docker buildx imagetools create -t ${MANIFESTIMAGE}:alpine-icewm -t ${MANIFESTIMAGE}:amd64-alpine-icewm -t ${MANIFESTIMAGE}:arm64v8-alpine-icewm ghcr.io/linuxserver/jenkins-builder:empty || true
