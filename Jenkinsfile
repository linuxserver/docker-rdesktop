        ...
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for PUSHIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  ...
                  docker buildx imagetools create -t ${PUSHIMAGE}:arch-mate ghcr.io/linuxserver/jenkins-builder:empty || true
                done
              '''
        }
      }
      ...
      steps {
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  docker buildx imagetools create -t ${MANIFESTIMAGE}:arch-mate -t ${MANIFESTIMAGE}:amd64-arch-mate -t ${MANIFESTIMAGE}:arm64v8-arch-mate ghcr.io/linuxserver/jenkins-builder:empty || true
