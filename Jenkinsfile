    ...
    stage("Set ENV Variables base"){
      steps{
        script{
          env.EXIT_STATUS = ''
          env.LS_RELEASE = sh(
            script: '''docker run --rm alexeiled/skopeo sh -c 'skopeo inspect docker://docker.io/'${DOCKERHUB_IMAGE}':i3-bionic 2>/dev/null' | jq -r '.Labels.build_version' | awk '{print $3}' | grep '\\-ls' || : ''',
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              docker run --rm -e CONTAINER_NAME=${CONTAINER_NAME} -e GITHUB_BRANCH=i3-bionic -v ${TEMPDIR}:/ansible/jenkins linuxserver/jenkins-builder:latest 
        ...
        withCredentials([
          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:i3-bionic
                  docker tag ${GITHUBIMAGE}:amd64-${META_TAG} ${GITHUBIMAGE}:${META_TAG}
                  docker tag ${GITHUBIMAGE}:arm32v7-${META_TAG} ${GITHUBIMAGE}:arm32v7-i3-bionic
                  docker tag ${GITHUBIMAGE}:arm64v8-${META_TAG} ${GITHUBIMAGE}:arm64v8-i3-bionic
                  ...
                  docker push ${GITHUBIMAGE}:i3-bionic
                  docker push ${GITHUBIMAGE}:${META_TAG}
                  docker push ${GITHUBIMAGE}:arm32v7-i3-bionic
                  docker push ${GITHUBIMAGE}:arm64v8-i3-bionic
