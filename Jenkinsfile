  ...
  stages {
    ...
    stage("Set tag custom bash"){
      steps{
        script{
          env.EXT_RELEASE = sh(
            script: ''' curl -sX GET https://api.github.com/repos/linuxserver/docker-baseimage-rdesktop/releases | jq -r 'first(.[] | select(.tag_name | startswith("arch-dbus"))) | .tag_name' | sed 's|arch-dbus-||' | sed 's|-ls.*||' ''',
            returnStdout: true).trim()
