        ...
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for PUSHIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  ...
                  docker buildx imagetools create -t ${PUSHIMAGE}:fedora-openbox ghcr.io/linuxserver/jenkins-builder:empty || true
                done
              '''
        }
      }
      ...
      steps {
        retry_backoff(5,5) {
          sh '''#! /bin/bash
                ...
                for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                  docker buildx imagetools create -t ${MANIFESTIMAGE}:fedora-openbox -t ${MANIFESTIMAGE}:amd64-fedora-openbox -t ${MANIFESTIMAGE}:arm64v8-fedora-openbox ghcr.io/linuxserver/jenkins-builder:empty || true
