...
Simply pulling `lscr.io/linuxserver/rdesktop:ubuntu-kde` should retrieve the correct image for your arch, but you can also pull specific arch images via tags.
...
docker run -d \
  ...
  --restart unless-stopped \
  lscr.io/linuxserver/rdesktop:ubuntu-kde
...
* image version number
  * `docker inspect -f '{{ index .Config.Labels "build_version" }}' lscr.io/linuxserver/rdesktop:ubuntu-kde`
...
* Update the image: `docker pull lscr.io/linuxserver/rdesktop:ubuntu-kde`
...
docker build \
  --no-cache \
  --pull \
  -t lscr.io/linuxserver/rdesktop:ubuntu-kde .
```
...
* **13.09.22:** - Migrate ubuntu-kde to s6v3
* **24.07.22:** - Install firefox from ppa for ubuntu-kde (only image with jammy right now)
